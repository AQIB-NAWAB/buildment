...
/**
 * MDX treats bare `{` and `<letter` outside of code as the start of a JS
 * expression / JSX tag. This prose was written as plain Markdown, so it's
 * full of harmless things like `{displayName}` in a quoted UI string or
 * `<1)` in "non-numeric, <1) -> 400" that are not meant to be JS/JSX at all.
 * Escape both, but only in prose — fenced code blocks and inline code spans
 * are left verbatim (they're safe already: remark treats their contents as
 * literal text, not JSX). `<Quiz .../>` / `<OpenQuestion .../>` placeholders
 * are inserted *after* this runs, so they're never escaped.
 */
function escapeMdxProse(body: string): string {
  return splitOutsideFences(body, /```[\s\S]*?```/g, (segment) =>
    splitOutsideFences(segment, /`[^`\n]+`/g, (prose) =>
      prose.replace(/[{}]/g, (ch) => (ch === "{" ? '{"{"}' : '{"}"}')).replace(/</g, "&lt;")
    )
  );
}
...
function parseLesson(moduleNum: number, moduleDir: string, fileName: string, lessonNum: number, slugPart: string): ParsedLesson {
  ...
  let source = transformQuizFences(body, pushBlock);
  ...
  return {
    ...
    source: source.trim() + "\n",
    blocks,
    isMilestone: /-checklist\.md$/.test(fileName),
  };
}
...
async function main() {
  ...
  for (const { dirName, moduleNum } of readModuleDirs()) {
    const title = CHAPTER_NAMES[moduleNum] ?? `Module ${moduleNum}`;
    const mod = await prisma.module.upsert({
      where: { courseId_order: { courseId: course.id, order: moduleNum } },
      update: { title },
      create: { courseId: course.id, order: moduleNum, title },
    });
    moduleCount += 1;
    ...
    for (const { fileName, lessonNum, slugPart } of readLessonFiles(dirName)) {
      ...
      const slug = `${moduleNum}-${lessonNum}-${slugPart}`;
