...
```predict
prompt: "A shopper's access token expires while their refresh token is still valid. What should FreshMarket do?"
options:
  - id: rotate-access
    label: "Verify the refresh token and issue a new short-lived access token"
  - id: extend-expired
