...
<FaqItem question="Why useRef vs useState for access token?">

Ref avoids re-rendering whole tree on silent refresh — either works; pick one pattern and stay consistent.

</FaqItem>

<FaqItem question="Can React read the refresh cookie to debug?">

httpOnly means **no** — use Network tab Set-Cookie headers instead.

</FaqItem>

<FaqItem question="What is `credentials: 'include'`?">

Tells fetch to send cookies on cross-origin calls — required for refresh from Vite to Express.
...
<OpenQuestion id="01KZ5P69RYY16Q2SSTGGWN39ZW" />
