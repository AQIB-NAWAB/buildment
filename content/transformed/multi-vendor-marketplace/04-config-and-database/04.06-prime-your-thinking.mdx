...
<BigWordAlert term="Twelve-factor config" plainEnglish="Store config in the environment, not in code — so dev, staging, and production differ by env vars only." whyItMatters="FreshMarket follows this pattern: one codebase, many deployments, secrets injected at runtime." />
...
<FaqItem question="Can I copy the Mongoose docs sample server verbatim?">
...
<FaqItem question="Should health use `readyState` or a boolean I set?">

Either is correct, as long as the health JSON always reflects the real connection state. `connection.readyState` is Mongoose's own answer — `0` disconnected, `1` connected, `2` connecting, `3` disconnecting — so reading it costs nothing and can't go stale. A boolean you set right after `connectDB()` succeeds is simpler, but it only captures the moment you connected; if the database dies later, that boolean still says "true." For a health check whose whole job is telling the truth, `readyState` is the more honest signal.

</FaqItem>

<FaqItem question="Why read disconnect if MVP never shuts down gracefully?">

Because "my dev machine never restarts" is a lie you'll meet in production, and disconnect is the flip side of connect — understanding it makes the whole connection model click. When a deploy platform restarts your server, it sends a shutdown signal; handling it with `mongoose.disconnect()` lets pending writes finish and closes the connection pool cleanly instead of dropping the database mid-request. It's optional for the gate, so don't stress, but skimming that section now means later you'll recognize what `SIGTERM` handlers and graceful shutdown are for.

</FaqItem>

<FaqItem question="What is \"Mongoose buffering timed out\" and why does it happen?">

It's Mongoose's way of saying "your code asked me to touch the database before I ever connected." While a connection is being established, Mongoose *buffers* operations — it queues them up, waiting for the connection to complete. If the connection never succeeds, those queued operations time out with that error, and it usually means your app tried to run a query (or started `listen`) before `mongoose.connect()` finished. The fix is the ordering this chapter keeps repeating: load env → validate config → connect → listen. If `MONGODB_URI` is wrong or the database is down, fail at boot with a clear error instead of mystery timeouts later.
...
<OpenQuestion id="01KZP7KP3Q4A764SNAY7CA34GB" />
