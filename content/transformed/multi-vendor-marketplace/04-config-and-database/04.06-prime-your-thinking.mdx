...
Adapt to your module layout — use `config.js` for URI, export `connectDB` from `shared/db.js`.

</FaqItem>

<FaqItem question="Should health use `readyState` or a boolean I set?">

Either if **accurate** — spec requires health JSON to reflect real connection state.

</FaqItem>

<FaqItem question="Why read disconnect if MVP never shuts down gracefully?">

Optional for gate — good habit for deploy restarts; note in learning log if you implement it.
...
<OpenQuestion id="01KZ5P690KDW1NPYKTKR6R8N0E" />
