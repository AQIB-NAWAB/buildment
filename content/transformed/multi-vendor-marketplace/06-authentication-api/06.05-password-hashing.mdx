...
Either works — native `bcrypt` is faster; `bcryptjs` pure JS if install fails on your OS.

</FaqItem>

<FaqItem question="Should I hash in Mongoose pre-save hook?">

Service layer is clearer for this course — one place register/login both trust.

</FaqItem>

<FaqItem question="Seed users have passwords before bcrypt — now what?">

Re-run seed after hashing or hash in seed script — document test password in README.

</FaqItem>

<FaqItem question="What is `select: false` on passwordHash?">

Prevents accidental hash in `/me` JSON — login query explicitly selects it.
...
<OpenQuestion id="01KZ5P69GS3XV00293DPSXQSD7" />
