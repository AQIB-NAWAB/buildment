...
Because those features need *somewhere to live*. Without `server/src/modules/` and a running Express app, every later chapter would say "add this somewhere" and you would paste into random files. Structure first; features attach to structure.

</FaqItem>

<FaqItem question="Do I build inside the course folder (`multi-vendor-marketplace/`)?">

No. That folder is the textbook. Your app lives in a **separate Git repo** you create in sub-chapter 2.8 — e.g. `~/projects/greenbasket/`.

</FaqItem>

<FaqItem question="I have never used Express or Vite. Is this chapter too early?">

This chapter introduces them **one small step at a time**. You will install packages, create entry files, and verify with curl and the browser. Unfamiliar errors are normal; the working rhythm from Chapter 1 applies: read the message, search the docs, fix the smallest failing piece.

</FaqItem>

<FaqItem question="Why two terminals — can't one command run everything?">

The client and server are **two separate programs** during development. Two terminals makes it obvious which half broke when something fails. You will run `npm run dev` in `server/` and again in `client/`.

</FaqItem>

<FaqItem question="What if I already have a MERN starter template?">

Unless it matches this course's **module-based** layout (`server/src/modules/…`), you will fight path instructions for twenty chapters. Safer to scaffold fresh here or adapt your repo to the target tree in 2.3.
...
<OpenQuestion id="01KZ5FW36R45E2D4DXKGZAWZ06" />
